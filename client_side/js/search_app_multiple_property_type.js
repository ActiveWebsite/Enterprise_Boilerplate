var ClientSearch = function (form_id, property_type, save_form, cma_form) {
	this.current_property_type = property_type;
...
ClientSearch.prototype = {
	...
	initAdvancedSearch: function () {
		...
		// build form controls that only need to be instantiated once
		this.buildSingletonFormControls();
		...
		this.form.find('.expand-block').each(function () {
			...
			if (!formBlock.hasClass('ignore')) {
				formBlock.find('input[type="radio"]:checked, input[type="checkbox"]:checked, input[type="text"], input[type="number"], select').each(function () {
					var el = jQuery(this), value = el.val(), isChecked = true, showit = false;
	...
	/**
	 * Contains any code that will instantiate controls on the search form once meaning it will not be called when a new form loads
	 */
	buildSingletonFormControls: function() {
		//handle open house radio buttons
		this.form.on('click', 'input[name="from"]', function () {
			var el = jQuery(this);
			if (el.hasClass('noOpenHouses')) {
				jQuery('#openHouseOn').val('');
			} else {
				jQuery('#openHouseOn').val('on');
			}
		});
		...
		this.form.on('click', '.prop_listed_by_field', function () {
			var el = jQuery(this),
				val = el.val(),
				agentBox = jQuery('.listing_agent_select_box');
			if (val === 'choose_agent') {
				agentBox.show();
			} else {
				agentBox.hide();
		...
		this.form.on('click', '.expand-trigger', function () {
			jQuery(this).toggleClass('active').next('.expand-block').slideToggle('fast');
		});
	},
	...
	/**
	 * Swaps out a form section based on the current property type
	 * requires the radio to have an action of data-action="change-property-type" and the template field name ie: data-template="residential"
	 */
	buildFormSwap: function () {
		var self = this;
		var propTypeButtons = this.form.find('input[data-action="change-property-type"]');
		propTypeButtons.on('click', function () {
			var cb = jQuery(this);
			var v = cb.val();
			var template = cb.data('template');
			var dynamicWrapper = jQuery('#dynamic-wrapper');
			var extraRequestdata = {};
			if (self.save_form) {
				extraRequestdata.save_button = true;
			}
			if (self.cma_form) {
				extraRequestdata.cma_form = true;
			}

			// if the radio is checked start the process;
			if (cb.is(':checked')) {
				if (v != self.current_property_type) {
					//disable counter
					if (self.counter) {
						self.counter.Stop();
					}
					...
					// disable the radios
					propTypeButtons.prop('disabled', true);
					
					// set the new type to the current type
					self.current_property_type = v;
					
					// if we have a new template enter this block
					if (template) {
						// remove old form and update with loading text
						jQuery('#dynamic-content').remove();
						dynamicWrapper.html('<p><img src="/images/ajax-loader.gif" alt=""> Loading new form...</p>');

						// get the new form
						jQuery.ajax({
							type: 'POST',
							url: '/search/getSearchFormViaAjax/search_form_prop_types|' + template,
							data: extraRequestdata,
							success: function (data) {
								dynamicWrapper.html('<div id="dynamic-content">' + data + '</div>');
								...
								// enable the radios
								propTypeButtons.prop('disabled', false);

								// rebuild form controls
								self.buildFormControls();

								// trigger form change to update count
								self.form.trigger('change');
							},
							error: function (data) {
								dynamicWrapper.html('<div id="dynamic-content"><div class="alert alert-error">Unable to load the form.</div></div>');
	...
	buildAutoComplete: function () {
		...
		this.form.find('.autoCompleteField').bind('keydown', function (event) {
			...
			source: function (request, responseFn) {
				...
				var el = jQuery(this.element).removeClass('ui-autocomplete-loading');
				var data_src = el.data('src');
				var search_list = self.autocompleteCache[data_src] || [];
				var last_term = extractLast(request.term);
				var search_term = jQuery.ui.autocomplete.escapeRegex(last_term); 
				...
				if (search_list && search_list.length) {
					matched_elements = jQuery.grep(search_list, function (item) {
					...
					jQuery.getJSON(restURL, function (data) {
						...
						if (data) {
							...
							matched_elements = jQuery.grep(dv, function (item) {
								return matcher.test(item);
							});
			...
			focus: function () {
				...
				jQuery(this).next('.invalid-auto-complete-entry').remove();
